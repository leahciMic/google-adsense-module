...
const adsbygoogle = {
  ...
  mounted () {
    console.log('adsense mounted')
    const detectIns = new MutationObserver(function (mutationRecords) {
      const hasNewIns = mutationRecords.some(({ addedNodes }) =>
        Array.from(addedNodes).some(node => node.nodeName === 'INS'));
      console.log('adsense detected ins, running adsense code')
      try {
    ...
    detectIns.observe(this.$el.parentNode, { childList: true });
    this.showAd()
  },
  watch: {
    '$route' (to, from) {
  ...
  methods: {
    ...
    showAd () {
      console.log('adsense showAd')
      console.log('adsense', window.adsbygoogle)
      console.log('adsense', this.$refs.ins)
      this.show = true
    }
  }
}
